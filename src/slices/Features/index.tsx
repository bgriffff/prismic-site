...
const compents: JSXMapSerializer = {
  heading2: ({ children }) => (
...
const Features = ({ slice }: FeaturesProps): JSX.Element => {
  return (
    ...
    >
      <PrismicRichText components={compents} field={slice.primary.heading} />
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 max-w-5xl gap-x-8 gap-y-12 mx-auto sm:place-items-start place-items-center">
        {slice.items.map((item, index) => (
          ...
          >
            ...
            <PrismicRichText components={compents} field={item.title} />
            <PrismicRichText components={compents} field={item.descriptions} />
          </div>
        ))}
      </div>
    </Bounded>
